...
function ex_serverconf()
{
  ...
  $free_space_on_data_path_drive = (gwmi win32_logicaldisk | where {$_.DeviceId -eq ($exchange_server.DataPath.DriveName)}).FreeSpace/1MB;
  $free_space_as_percentage      = $free_space_on_data_path_drive / ((gwmi win32_logicaldisk | where {$_.DeviceId -eq ($exchange_server.DataPath.DriveName)}).Size/1MB) * 100.00;
